...
- **Si le test echoue** → **TOUJOURS deleguer a `debug-unity` AVANT de retoucher au code**. L'agent investigue (4 phases : root cause → pattern → hypothesis → fix plan), retourne un diagnostic + plan de fix. SEULEMENT ENSUITE, dispatcher au bon implementer (`dev-unity` ou `refacto-unity`) avec le fix plan en contexte. Relancer le test jusqu'a ce que ca passe. Si 3 fix plans successifs echouent → STOP, escalade architecturale (cf rapport `debug-unity`).

  **Branche "contract-change" — modification de test autorisee sans validation utilisateur** :
  - Cas : la sous-tache courante de l'Issue **change explicitement le contrat write-side** (nouvelle valeur de retour, nouvel arrondi, nouvelle borne, nouvelle precision). Un test existant asserte l'ANCIEN contrat et echoue mecaniquement.
  - **Diagnostic clair** (heads-up `dev-unity` ou trace `test-play-unity` qui montre exactement la divergence numerique attendue par le nouveau contrat — ex : `expected 30, got 31` apres un changement explicite de Step) → le lead dispatch **directement `dev-unity` ou `refacto-unity`** avec un fix plan : "mettre a jour l'assertion pour cibler le NOUVEAU contrat". **Ne pas demander a l'utilisateur.** **Ne pas dispatcher `debug-unity`** quand le diagnostic est deja sans ambiguite.
  - **Diagnostic ambigu** (pas sur si c'est un changement de contrat ou une vraie regression) → dispatch `brainstorm-unity` pour une confirmation 1-question (formuler : "test X asserte l'ancien contrat A, l'Issue change pour B, ok pour re-cibler ?"), **PAS l'utilisateur**. Brainstorm tranche, le lead enchaine.
  - **Memoire `feedback_never_modify_tests`** : la regle interdit de **loosening / weakening** une assertion pour la rendre verte sans comprendre pourquoi (cacher un bug). Elle **n'interdit PAS** de re-cibler une assertion vers le NOUVEAU contrat explicitement demande par l'Issue : la stricture reste identique, seule la valeur asserted change. Cas OK : "API signature changed", "behavior deliberately changed", "feature removed". Cas KO : "weaken tolerance without justification", "change values just to make it green without understanding why".
  - **Cas non contract-change** (test casse par effet de bord, regression non voulue) → la regle stricte s'applique : on corrige le code, jamais le test. Ce n'est PAS une branche contract-change.
